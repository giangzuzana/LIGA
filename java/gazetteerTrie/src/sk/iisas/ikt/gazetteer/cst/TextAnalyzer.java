...
public class TextAnalyzer {
    ...
    public static void main(String[] args) {
    	...
    	String searchFile = "data/_reuters-concat-9909.xml";
    	...
    	System.out.println("gazetteerCharCST " + searchFile + " " + parserFile +"\n");
    ...
    private void handleInput(PushbackReader reader) {
        ...
        try {
            while ((charInt = reader.read()) != -1) {			
                ...
                TreeNode nextNode = findChildWithChar(currentNode, readingChar);	// znak je v strome
                if (nextNode != null) {             	
                	...
                	if (currentNode.isListNode() ) {								// ak node je list  tak je matched entity						 
                        ...
                        if (!isDigitLetter(readingChar)) {							// ak to nie je pismeno/cislo, tak je to naozaj matched entity
                            addNewValueToCountHashMap(currentNode);
                	...
                	// ak sme vnorenim stromom presli cez white space, tak sa treba vratit k prvemu vyskytu medzery
                    if ((readingString.length() > 0 ) && readingString.contains(" "))	{
                    	...
                    	if (backString.trim().length() > 0) {
                    		char[] backCharArray = backString.toCharArray();
                       		...
                       		if (readingChar != '\n' && readingChar != '\r')		// kvoli malovankam
