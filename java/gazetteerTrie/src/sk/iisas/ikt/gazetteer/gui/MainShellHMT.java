...
import sk.iisas.ikt.gazetteer.hmt.TextAnalyzer;
import sk.iisas.ikt.gazetteer.hmt.ListParser;
import sk.iisas.ikt.gazetteer.hmt.TreeNode;
...
public class MainShellHMT {
	...
	public MainShellHMT(Display display) {
	...
	public void initUI() {
		...
		textFilePathText.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				MainShellHMT.this.textFilePathStr = ((Text) e.widget).getText();
			}
		});
		...
		chooseTestFileButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				...
				if (path != null) {
					MainShellHMT.this.textFilePathText.setText(path);
				}
			}
		});
		...
		listFilePathText.addModifyListener(new ModifyListener() {
			...
			@Override
			public void modifyText(ModifyEvent e) {
				MainShellHMT.this.listFilePathStr = ((Text) e.widget).getText();
			}
		});
		...
		chooseListFileButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				...
				String path = dialog.open();
				if (path != null) {
					MainShellHMT.this.listFilePathText.setText(path);
				}
			}
		});
		...
		searchButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				...
				if (!MainShellHMT.this.listFilePathStr.equals("")
						&& !MainShellHMT.this.textFilePathStr.equals("")) {
					TreeNode rootHashMap = MainShellHMT.this.listParser
							.parseListToTree(MainShellHMT.this.listFilePathStr);

					MainShellHMT.this.textAnalyzer.setRootNode(rootHashMap);
					MainShellHMT.this.textAnalyzer.searchText(MainShellHMT.this.textFilePathStr);

					Iterator<Entry<TreeNode, Integer>> it = MainShellHMT.this.textAnalyzer.getCountHashMap().entrySet().iterator();
                    ...
                    Double fileSize = new Double(TextAnalyzer.getSizeOfFile(MainShellHMT.this.textFilePathStr));
                    if (fileSize.compareTo(new Double(20)) > 0) {
                        ...
                        textOutcome.setText(TextAnalyzer
                                .readTextFileToString(MainShellHMT.this.textFilePathStr));
